      ...
      <div class="col-xs-12 col-sm-6 col-md-3 d-flex pb-3" v-for="board in unarchivedBoards" :key="board.id">
        <div class="card w-100 board-item">
          ...
          <div class="card-footer bg-transparent">
            <router-link class="btn btn-sm btn-primary mr-2 mb-1" :to="{ name: 'task-board', params: { id: board.id } }"
              >View</router-link
            >
            <button class="btn btn-sm btn-info mb-1" @click="handleArchiveTaskBoard(board)">
              Archive
            </button>
          </div>
        </div>
